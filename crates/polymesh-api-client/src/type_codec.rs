...
impl TypeLookup {
  pub fn decode_value<I: Input>(&self, type_id: TypeId, input: &mut I, is_compact: bool) -> Result<Value> {
    let ty = self.get_type(type_id).ok_or_else(|| Error::DecodeTypeFailed(format!("Missing type_id: {type_id:?}")))?;
    ty.decode_value(self, input, is_compact)
  }
}

impl Type {
  pub fn decode_value<I: Input>(&self, type_lookup: &TypeLookup, input: &mut I, is_compact: bool) -> Result<Value> {
        ...
        match prim {
          TypeDefPrimitive::Bool => {
            match input.read_byte()? {
              0 => Ok(json!(false)),
              1 => Ok(json!(true)),
              num => Err(Error::DecodeTypeFailed(format!("Invalid bool byte: {num:?}")))
            }
          },
...
fn decode_fields<I: Input>(fields: &Vec<Field>, type_lookup: &TypeLookup, input: &mut I, is_compact: bool) -> Result<Value> {
  let len = fields.len();
  if len == 0 {
  ...
  match fields.len() {
    ...
    len => {
      ...
      for (idx, field) in fields.iter().enumerate() {
        let name = field.name.as_ref().cloned().unwrap_or_else(|| format!("{idx}"));
...
impl TypeDefVariant {
  pub fn decode_value<I: Input>(&self, type_lookup: &TypeLookup, input: &mut I, is_compact: bool) -> Result<Value> {
    ...
    match self.get_by_idx(val) {
      Some(variant) if variant.fields.len() == 0 => {
        Ok(json!(variant.name))
      }
      Some(variant) => {
        let mut m = Map::new();
        let name = variant.name.clone();
        m.insert(name, decode_fields(&variant.fields, type_lookup, input, is_compact)?);
        Ok(m.into())
      }
      None if val == 0 => Ok(Value::Null),
      None => {
        log::debug!("Invalid variant: {}, bytes remaining: {:?}, variants: {:?}", val, input.remaining_len()?, self.variants);
...
impl TypeDefTuple {
  pub fn decode_value<I: Input>(&self, type_lookup: &TypeLookup, input: &mut I, is_compact: bool) -> Result<Value> {
    match self.fields.len() {
      ...
      1 => {
        Ok(type_lookup.decode_value(self.fields[0], input, is_compact)?)
      }
      len => {
        let mut vec = Vec::with_capacity(len);
...
impl TypeDefCompact {
  pub fn decode_value<I: Input>(&self, type_lookup: &TypeLookup, input: &mut I, _is_compact: bool) -> Result<Value> {
